...
void compare_algorithms(const char *file_name) {
    ...
    Position *positions = get_positions_from_file(pos_file_name,
                                                  &size_positions);
    int size_employees = 0;
    Employee *employees = get_employees_from_file(file_name,
                                                  &size_employees);

    Average_salary *aver_salaries_simple = count_average_salaries(
            employees, size_employees, positions, size_positions);

    unsigned int time2 =
            duration_cast<milliseconds>(system_clock::now().time_since_epoch())
                    .count();
    Average_salary *aver_salaries_parallel = parallel_count_average_salaries(
...
TEST(parallel_get_k_employees_from_file, null_input) {
    EXPECT_TRUE(get_k_employees_from_file(nullptr, 6) == nullptr);
}
...
TEST(parallel_count_average_salary, count_average) {
    ...
    Position *positions = get_positions_from_file(
            "project/test/data/positions.txt", &size_positions);
    auto *count_aver = (Count_average *)malloc(sizeof(Count_average) * 6);
    for (int i = 0; i < 6; i++) {
        count_aver[i].count = 10;
        count_aver[i].total = 1000;
    }
    Average_salary *aver_salary =
...
TEST(parallel_count_salaries_for_positions, count_average) {
    int size_positions = 0;
    Average_salary *aver_salary = parallel_count_average_salaries(
            "project/test/data/input_1000.txt",
            "project/test/data/positions.txt",
            &size_positions);
    EXPECT_EQ(size_positions, 6);
